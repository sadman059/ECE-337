...
module adder_nbit ( a, b, carry_in, sum, overflow );
  ...
  adder_1bit_3 \genblk1[0].ADDi  ( .a(a[0]), .b(b[0]), .carry_in(carry_in), 
        .sum(sum[0]), .carry_out(carrys[1]) );
  adder_1bit_2 \genblk1[1].ADDi  ( .a(a[1]), .b(b[1]), .carry_in(carrys[1]), 
        .sum(sum[1]), .carry_out(carrys[2]) );
  adder_1bit_1 \genblk1[2].ADDi  ( .a(a[2]), .b(b[2]), .carry_in(carrys[2]), 
        .sum(sum[2]), .carry_out(carrys[3]) );
  adder_1bit_0 \genblk1[3].ADDi  ( .a(a[3]), .b(b[3]), .carry_in(carrys[3]), 
